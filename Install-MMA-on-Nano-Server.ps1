...
new-PSSession -ComputerName $NanoServer -Credential (Get-Credential)
$TargetSession = new-PSSession -ComputerName $NanoServer -Credential (Get-Credential)
Copy-Item -Path ".\MMASetup-AMD64.exe" -Destination "C:\" -force -Recurse -ToSession $TargetSession
Enter-PSSession $TargetSession
C:\MMASetup-AMD64.exe /Q:A /R:N /C:"setup.exe /qn ADD_OPINSIGHTS_WORKSPACE=1 OPINSIGHTS_WORKSPACE_ID=$OPSINSIGHTS_WS_ID OPINSIGHTS_WORKSPACE_KEY=$OPSINSIGHTS_WS_KEY AcceptEndUserLicenseAgreement=1"
